...
function inferProcessConfigType(fileName: string, fileType?: string) {
  if (fileType && fileType !== "undefined" && fileType !== "null") return fileType;
  const lowerName = String(fileName).toLowerCase();
  if (lowerName === "server.properties" || lowerName.endsWith(".properties")) return "properties";
  if (lowerName.endsWith(".yml") || lowerName.endsWith(".yaml")) return "yml";
...
function createOfflineUuid(name: string) {
  const source = Buffer.from(`OfflinePlayer:${name}`, "utf8");
  const bytes = crypto.createHash("md5").update(source).digest();
...
function readWhitelistConfig(instance: Instance, filePath: string) {
  ...
  const entries: WhitelistEntry[] = rawList.map((item, index) => {
    const name = String(item?.name || "").trim();
    const uuid = String(item?.uuid || "").trim().toLowerCase();
    const offlineUuid = name ? createOfflineUuid(name).toLowerCase() : "";
    const isOfflineUuid = !!name && !!uuid && uuid === offlineUuid;
    const duplicateName = !!name && (nameCounter.get(name) || 0) > 1;
    ...
    else if (uuid) source = "正版/外部 UUID";

    let note = "";
    if (duplicateName) note = "同名条目不会自动合并，请按 UUID 区分身份。";
    else if (isOfflineUuid) note = "此条目在 online-mode=false 时生效。";
    else if (uuid) note = "此条目按真实/外部 UUID 识别。";
...
function writeWhitelistConfig(instance: Instance, filePath: string, payload: Record<string, any>) {
  ...
  fs.writeFileSync(filePath, JSON.stringify(rawList, null, 4) + "\n", { encoding: "utf-8" });
}
...
routerApp.on("instance/process_config/file", (ctx, data) => {
  ...
  const fileType = inferProcessConfigType(fileName, data.type);
  try {
    const instance = InstanceSubsystem.getInstance(instanceUuid);
    if (!instance) throw new Error($t("TXT_CODE_3bfb9e04"));
    const fileManager = new FileManager(instance.absoluteCwdPath());
